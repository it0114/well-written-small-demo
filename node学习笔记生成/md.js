...
const generateDiaryOutline = (year, path) => {
    createYearFolder(year, path)
}

/**
 @param year: 指定年份，生成对应年份的日记框子
 @param path: 指定路径，指定文件夹生成到对应路径
 */
generateDiaryOutline()
...
function createYearFolder(year = new Date().getFullYear(), path) {
    ...
    if (path) {
        ...
        yearFolderPath = `${__dirname}/${year}`  // 当前文件夹下创建
    }
